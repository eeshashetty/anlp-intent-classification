...
class BertHierarchicalEvaluator(object):
    ...
    def get_scores(self, silent=False):
        ...
        self.save_to_df(target_labels_fine, predicted_labels_fine, target_labels_coarse, predicted_labels_coarse)
        print("SAVED TO ", self.metrics_save_path)
    ...
    def save_to_df(self, target_labels_fine, predicted_labels_fine, target_labels_coarse, predicted_labels_coarse):
        guid = [f.guid for f in self.eval_examples]
        text_a = [list(map(int, f.text_a.split())) for f in self.eval_examples]
        text_b = [f.text_b for f in self.eval_examples]
        text_c = [f.text_c for f in self.eval_examples]
        text_d = [f.text_d for f in self.eval_examples]
        label = [f.label for f in self.eval_examples]
        ...
        df = pd.DataFrame({
            ...
            "text_d": text_d, 
            "label": label, 
            "target_labels_fine": target_labels_fine,
            "predicted_labels_fine": predicted_labels_fine,
            "target_labels_coarse": target_labels_coarse,
            "predicted_labels_coarse": predicted_labels_coarse})

        df.to_csv(self.metrics_save_path)
