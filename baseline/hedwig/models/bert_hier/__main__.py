import sys
sys.path.append('/home/ubuntu/subjective_discourse/code/models/hedwig')
sys.path.append('/home/ubuntu/subjective_discourse/code/models/hedwig/models')
sys.path.append('/home/ubuntu/subjective_discourse/code/models/hedwig/models/bert_hier')
import random
import json

import numpy as np
import torch
from transformers import BertForSequenceClassification, RobertaForSequenceClassification, AdamW, BertTokenizer, RobertaTokenizer, XLNetTokenizer, get_linear_schedule_with_warmup
...
def run_main(args):
    ...
    if args.model == "nlpaueb/legal-bert-base-uncased":
        tokenizer= tokenizer = AutoTokenizer.from_pretrained("nlpaueb/legal-bert-base-uncased")
    else:
    ...
    if not args.trained_model:
        ...
        if args.evaluate_train:
            evaluate_split(model, processor, tokenizer, args, metrics_dev_json, split='train')
